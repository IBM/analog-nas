...
class ResidualBranch(nn.Module):
    def __init__(self, in_channels, out_channels, filter_size, stride, branch_index):
        ...
        self.residual_branch.add_module(
            ...
            nn.Conv2d(out_channels, out_channels, filter_size, stride=1, padding=round_(filter_size), bias=False),
        )
...
class SkipConnection(nn.Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.s1.add_module(
            "Skip_1_Conv",
            nn.Conv2d(in_channels, int(out_channels / 2), kernel_size=1, stride=1, padding=0, bias=False),
        )
...
class BasicBlock(nn.Module):
    def __init__(self, n_input_plane, n_output_plane, filter_size, res_branches, stride):
        ...
        self.branches = nn.ModuleList(
            [
                ResidualBranch(n_input_plane, n_output_plane, filter_size, stride, branch + 1)
        ...
        if n_input_plane != n_output_plane or stride != 1:
            self.skip.add_module("Skip_connection", SkipConnection(n_input_plane, n_output_plane, stride))
...
class ResidualGroup(nn.Module):
    def __init__(self, block, n_input_plane, n_output_plane, n_blocks, filter_size, res_branches, stride):
        ...
        self.group.add_module("Block_1", block(n_input_plane, n_output_plane, filter_size, res_branches, stride=1))
        ...
        for block_index in range(2, n_blocks + 1):
            ...
            self.group.add_module(
                block_name, block(n_output_plane, n_output_plane, filter_size, res_branches, stride=1)
            )
...
class Network(nn.Module):
    def __init__(self, config, input_dim=(3, 32, 32), classes=10):
        ...
        self.blocks.add_module(
            "Conv_0", nn.Conv2d(3, config["out_channel0"], kernel_size=7, stride=1, padding=1, bias=False)
        )
