    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, depth, padding='same'):
        ...
        for i in range(depth):
            ...
            deconv = nn.ConvTranspose2d(
                ...
                stride=stride if i == 0 else 1,
                padding=kernel_size // 2 if padding == 'same' else 0,
                output_padding=(stride - 1 if i == 0 else 0)  # To better control the spatial resolution
            )
...
class AutoEncoder(nn.Module):
    def __init__(self, config, input_channels=3, input_size=(32,32)):
        ...
        with torch.no_grad():
            ...
            self.encoded_shape = encoded_feat.shape[1:]
        ...
        decoder_blocks = []
        for i in range(1, 4):
            # depth = config[f"decoder_convblock{i}_depth"]
