...
def main():
    ...
    for i in tqdm(range(len(paths))):
        ...
        path = paths[i]
        img = cv2.imread(
            os.path.join(path, "images", os.path.basename(path) + ".png")  #
        )
        ...
        cv2.imwrite(
            os.path.join(
                "inputs/dsb2018_%d/images" % img_size, os.path.basename(path) + ".png"
            ),
            img,
        )
        cv2.imwrite(
            os.path.join(
                "inputs/dsb2018_%d/masks/0" % img_size, os.path.basename(path) + ".png"
            ),
